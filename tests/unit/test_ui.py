...
class TestLobbyCallbacks:
    def test_lobby_ready(self):
        ...
        deps.user_repo.save_user({"userId": "u1", "currentLobbyId": res.lobby["lobbyId"]})
    ...
    def test_lobby_start(self):
        ...
        res = deps.lobby_manager.create_lobby("u1", "c1")
        lobby_id = res.lobby["lobbyId"]
        deps.user_repo.save_user({"userId": "u1", "currentLobbyId": lobby_id, "chatId": "c1"})
        
        deps.lobby_manager.join_lobby("u2", res.lobby["code"])
        deps.user_repo.save_user({"userId": "u2", "currentLobbyId": lobby_id, "chatId": "c2"})
